...
describe('Canvas CLI Integration', () => {
  it('should register canvas commands', () => {
    ...
    const canvasCommand = program.commands.find((cmd: Command) => cmd.name() === 'canvas');
  ...
  it('should register session subcommands', () => {
    const program = createProgram();
    const canvasCommand = program.commands.find((cmd: Command) => cmd.name() === 'canvas');
    const sessionCommand = canvasCommand?.commands.find((cmd: Command) => cmd.name() === 'session');

    expect(sessionCommand).toBeDefined();
    expect(sessionCommand?.description()).toBe('Canvas session management');

    const sessionSubcommands = sessionCommand?.commands.map((cmd: Command) => cmd.name());
  ...
  it('should register queue subcommands', () => {
    const program = createProgram();
    const canvasCommand = program.commands.find((cmd: Command) => cmd.name() === 'canvas');
    const queueCommand = canvasCommand?.commands.find((cmd: Command) => cmd.name() === 'queue');

    expect(queueCommand).toBeDefined();
    expect(queueCommand?.description()).toBe('Canvas queue management');

    const queueSubcommands = queueCommand?.commands.map((cmd: Command) => cmd.name());
  ...
  it('should register agent subcommands', () => {
    const program = createProgram();
    const canvasCommand = program.commands.find((cmd: Command) => cmd.name() === 'canvas');
    const agentCommand = canvasCommand?.commands.find((cmd: Command) => cmd.name() === 'agent');

    expect(agentCommand).toBeDefined();
    expect(agentCommand?.description()).toBe('Canvas agent interaction');

    const agentSubcommands = agentCommand?.commands.map((cmd: Command) => cmd.name());
  ...
  it('should register run subcommands', () => {
    const program = createProgram();
    const canvasCommand = program.commands.find((cmd: Command) => cmd.name() === 'canvas');
    const runCommand = canvasCommand?.commands.find((cmd: Command) => cmd.name() === 'run');

    expect(runCommand).toBeDefined();
    expect(runCommand?.description()).toBe('Canvas run management');

    const runSubcommands = runCommand?.commands.map((cmd: Command) => cmd.name());
  ...
  it('should register scheduling subcommands', () => {
    const program = createProgram();
    const canvasCommand = program.commands.find((cmd: Command) => cmd.name() === 'canvas');
    const heartbeatCommand = canvasCommand?.commands.find((cmd: Command) => cmd.name() === 'heartbeat');
    const cronCommand = canvasCommand?.commands.find((cmd: Command) => cmd.name() === 'cron');
